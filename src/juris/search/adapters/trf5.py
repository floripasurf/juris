"""TRF5 (Tribunal Regional Federal da 5ª Região) search adapter."""
...
_PORTAL_URL = "https://trf5.jus.br/cp/pesquisar"
_URL_PREFIX = "https://trf5.jus.br"

_CNJ_PATTERN_RE = __import__("re").compile(r"\d{7}-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}")


@register_adapter
class TRF5Adapter(SearchAdapter):
    """Adapter for TRF5 jurisprudência HTML search portal."""

    court_code: ClassVar[str] = "trf5"
    ...
    async def search(self, query: SearchQuery) -> list[SearchResult]:
        """Search TRF5 jurisprudência portal.
        ...
        try:
            ...
            logger.warning("TRF5 search failed: %s", safe_error_text(exc))
            return []

    def _parse(self, html: str, query: SearchQuery) -> list[SearchResult]:
        """Parse HTML table response from TRF5 portal.
        ...
        for table in soup.find_all("table"):
            for row in table.find_all("tr"):
                ...
                try:
                    ...
                    logger.debug("TRF5: failed to parse row: %s", safe_error_text(exc))
                    continue
