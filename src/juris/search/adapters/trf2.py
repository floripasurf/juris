...
@register_adapter
class TRF2Adapter(SearchAdapter):
    """Adapter for TRF2 jurisprudência HTML search portal."""

    court_code: ClassVar[str] = "trf2"
    ...
    async def search(self, query: SearchQuery) -> list[SearchResult]:
        """Search TRF2 jurisprudência portal.
        ...
        try:
            ...
            logger.warning("TRF2 search failed: %s", safe_error_text(exc))
            return []

    def _parse(self, html: str, query: SearchQuery) -> list[SearchResult]:
        """Parse HTML table response from TRF2 portal.
        ...
        for table in soup.find_all("table"):
            for row in table.find_all("tr"):
                ...
                try:
                    ...
                    logger.debug("TRF2: failed to parse row: %s", safe_error_text(exc))
                    continue
