"""Ingester for CNJ petition template documents.

GATED: Requires manual URL curation in data/cnj_modelos_index.json by a lawyer.
"""
...
_DEFAULT_INDEX = Path(__file__).resolve().parents[4] / "data" / "cnj_modelos_index.json"


class CNJModelosIngester(CorpusIngester):
    """Ingests CNJ petition template documents from curated URLs.

    Rate limit: ≤1 req/2sec, respects robots.txt.
    Target sources: CNJ modelos de petição, formulários padronizados.
    ...
    def fetch(self) -> list[FonteJurisprudencia]:
        """Read curated CNJ template URLs and fetch content.

        Returns:
            List of FonteJurisprudencia, empty until URLs are curated.
        """
        if not self._index_path.exists():
            logger.warning("CNJ modelos index not found: %s", self._index_path)
            return []
        ...
        if not entries:
            logger.info("CNJ modelos index is empty — awaiting manual URL curation")
            return []

        # TODO: Implement HTTP fetching once URLs are curated
        logger.info(
            "CNJ modelos index has %d entries — HTTP fetching not yet implemented",
            len(entries),
        )
        return []
