...
class TestTJSPAdapterParseFixture:
    ...
    def test_parse_cnj_normalized_first(
        self, adapter: TJSPAdapter, fixture_html: str, query: SearchQuery
    ) -> None:
        results = adapter._parse(fixture_html, query)
    ...
    def test_parse_url_has_esaj_prefix(
        self, adapter: TJSPAdapter, fixture_html: str, query: SearchQuery
    ) -> None:
        results = adapter._parse(fixture_html, query)
        assert results[0].url.startswith("https://esaj.tjsp.jus.br")

    def test_parse_court_is_tjsp(
        self, adapter: TJSPAdapter, fixture_html: str, query: SearchQuery
    ) -> None:
        results = adapter._parse(fixture_html, query)
        for r in results:
...
class TestTJSPAdapterParseEdgeCases:
    def test_parse_no_table_returns_empty(
        self, adapter: TJSPAdapter, query: SearchQuery
    ) -> None:
    ...
    def test_parse_respects_max_results(
        self, adapter: TJSPAdapter, fixture_html: str
    ) -> None:
        q = SearchQuery(
...
class TestTJSPAdapterSearch:
    ...
    async def test_search_get_error_returns_empty(
        self, adapter: TJSPAdapter, query: SearchQuery
    ) -> None:
    ...
    async def test_search_post_error_returns_empty(
        self, adapter: TJSPAdapter, query: SearchQuery
    ) -> None:
        mock_get_resp = MagicMock()
    ...
    async def test_search_passes_viewstate_in_post(
        ...
        form_data = call_kwargs.kwargs.get("data") or call_kwargs.args[1] if call_kwargs.args[1:] else {}
        assert "javax.faces.ViewState" in str(call_kwargs)

    async def test_search_none_viewstate_still_posts(
        self, adapter: TJSPAdapter, query: SearchQuery
    ) -> None:
