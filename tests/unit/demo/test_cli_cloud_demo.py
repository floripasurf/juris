...
def test_demo_cli_cloud_passes_subscription_adapter_and_audits_provider(tmp_path: Path) -> None:
    ...
    assert cast(LocalCliLLM, captured["llm"]).model_name == "claude_cli_subscription"
...
def test_demo_cli_cloud_rejects_real_source(tmp_path: Path) -> None:
    result = runner.invoke(
        app,
        [
            "demo",
    ...
    assert result.exit_code == 1, result.output
    assert "--cli-cloud só aceita source=fixture" in result.output
